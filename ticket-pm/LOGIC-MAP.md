...
- 值日专用群（`DUTY_CHAT_ID`，快递申领群）值日分支先于一切能力（`handleDutyBranch`）：p2p 值日指令/图片直转 duty-bot；群内 @消息「值日助手」出看板、关键词命中照常回答（同款 `buildMentionReplyForText`），其余回值日群引导语，基础指令关闭；未@消息照常走管道③（值日群在关键词回答范围内）。
