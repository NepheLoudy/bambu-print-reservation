...
群聊消息：  ① chatService（必须@机器人；值日管辖群分支先行：看板词/值日指令/图片转 duty-bot
                           ...
                           切换为 /approval-*；@我时非指令命中 → 先查 抽奖触发词（最优先，2026-09-14），未命中再查 @触发回答表，仍未命中回落 关键词回答表）
            ② DDL逾期确认(handleReply，仅4个播报群；确认必须来源匹配：p2p发的只能私聊回、群发的只能同群回)
            ③ 抽奖·lottery（未@消息命中 lottery.json 触发词即抽一次，2026-09-14；LOTTERY_CHAT_IDS 收窄，留空=全群；
                           命中即跳过③′关键词回答——同一条消息不双回，发言记录④照常）
...
每一环 handled 即终止管道；DDL 确认"未识别回复"在群聊静默放行给后续环节；③/③′ 抽奖或自动回复命中也不终止（发言记录照常往下走）。
...
- 抽奖（`lotteryService`，2026-09-14）：`抽奖配置表.xlsx`「抽奖配置」工作表（触发词/奖品/概率1…，`scripts/syncLottery.js` → `lottery.json`，解析复用 syncAutoReplies）——未@群消息**包含**触发词即按概率加权随机抽一条奖品原文引用回复；@机器人时同样命中且**优先级最高**（> @触发回答 > 关键词回答，命中互斥不双回）；私聊不触发；`LOTTERY_CHAT_IDS` 可收窄（留空=全群）。窗口 `/api/lottery/rules*`（写端点 X-API-Token）热改 `.local.json`；`/lottery` 指令看奖池；命中上报统计 feature=抽奖。属对话回路，不受晚间静默限制。
