...
播报一次考勤打卡数据（周报卡 + CSV 明细附件）、打卡数据拉取与聚合、成员名单窗口管理。
关键词：考勤、打卡周报（企业微信侧）。

- 数据源=企微打卡报表人肉周导（POST /api/attendance/import，X-API-Token；名单从报表自动合并，无需手工维护）。企微 API 链路保留备用（凭据已配，仅差可信IP 门槛），ATTENDANCE_DATA_SOURCE 可切。

## 归属裁定

- **数据源在企微、播报通道可双栈**（2026-09-15 v3 起）：拉数只走企微开放接口
  （`qyapi.weixin.qq.com`，打卡数据+自建应用）；播报走群自定义机器人 webhook——
  **飞书群机器人为主通道**（`FEISHU_WEBHOOK_URL`，duty-bot 看板卡同款链路），
  企微群机器人为可选通道；CSV 可经现有飞书应用 im API 发文件；
- **webhook 是出站播报，不是消息消费**：不消费飞书消息事件、不接 feishu-gateway、
  不在网关登记路由、不受「对话铁律」「队员/功能统计上报」约束（duty-bot 看板卡同先例）；
