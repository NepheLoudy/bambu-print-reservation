...
- **网关日活跃**（唯一活跃表；**口径=机器人交互**：显式路由命中/私聊/@机器人，群内未 @ 闲聊不计）：gateway `src/bitable-sync.js` 每 30 分钟按日期签名 upsert（数据没变不写表），`POST /api/usage-sync/run?force=1` 手动补数；「网关功能使用/网关队员活跃」两表已下线（2026-09-13 用户拍板监听只留机器人交互强相关，建表脚本同步移除，勿重建）；
