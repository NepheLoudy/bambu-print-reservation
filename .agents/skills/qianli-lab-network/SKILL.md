...
## 十一、裁判系统路由器（已配置待部署，2026-09-20）

**术语先行**：本节与《网关拓扑文档.md》中"机器人"=与裁判系统连接的**物理机器人**（机甲大师场外）；开发内网 31.x 上的六仓"机器人"=飞书软件机器人服务，两者无任何关联。

- **状态：已配置、未部署**。机型待补（小米系），LAN `192.168.3.1/24`、WiFi `裁判系统`（密码用户 2026-09-20 设，不写入 git）均已在手机上配好；**当前未插线**（用户 2026-09-20 拔走）。部署=其 wan 插主网 LAN，自动 DHCP。
