...
## 十一、裁判系统路由器（第三网段，2026-09-20 入网）

**术语先行**：本节与《网关拓扑文档.md》中"机器人"=与裁判系统连接的**物理机器人**（机甲大师场外）；开发内网 31.x 上的六仓"机器人"=飞书软件机器人服务，两者无任何关联。

- **定位**：机甲大师裁判系统子网（物理机器人 + 裁判端电脑专用）。LAN `192.168.3.1/24`，wan=DHCP **已插主网**（实测 `192.168.31.80`，MAC `CC:C4:B2:46:16:2F`，在线稳定）；WiFi `裁判系统` 已启用（密码用户 2026-09-20 设，不写入 git）。品牌型号待补，管理入口 `192.168.3.1`（手机连裁判系统 WiFi 或任意 3.x 设备）。
- **双端联通状态**：物理机器人→小电脑(31.57) **已全通**（裁判路由器 NAT 出站 + 小电脑防火墙入站规则 `Referee-Zone-192.168.3.0-24` 全协议放行，2026-09-20 经 SSH 加注）；小电脑→物理机器人 🟡 待裁判系统官方软件发布后在裁判路由器加端口转发（主网关 miwifi 无静态路由功能，3.x 无回程路由，PC 主动发起默认不通）。
- **待固化**：其 wan 建议在自身后台把上网方式从 DHCP 改**静态 IP**（192.168.31.80 / 255.255.255.0 / 网关与 DNS 192.168.31.1）——与主网关静态租约等价且不用动主网关；miwifi 的静态租约 API 端点未公开，客户端侧钉死更稳。
