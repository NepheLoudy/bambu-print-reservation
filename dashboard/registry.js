...
module.exports = {
  ...
  projects: [
    ...
    },
    {
      ...
      role: '所有对话/指令的唯一入口：@对话、基础指令、关键词监听与自动回答、抽奖（/触发词 动态指令）、DDL 播报与逾期确认、会议提醒、项目表；专项指令转发各服务。',
      listening: [
        '全部消息事件（gateway 转发）；群聊需 @机器人，私聊直接对话',
        'p2p：DDL 逾期确认回复（12 小时时效，超时次日播报重问）→ 值日分支（指令/图片，duty-bot 未接管时落回常规流）→ 基础指令（白名单）→ 对话',
        '群：@对话 → 值日群分支（看板仅 @ 触发、带不带 / 均可、@+纯图片静默、非管辖群值日指令回提示）→ 审批群分支 → 基础指令（静态表未命中接 抽奖动态指令 /触发词）→ 关键词自动回答（全群统一，未@也生效）；会议提醒对值日管辖群跳过（群级功能全关）',
        'GET /api/hub/policy（定制窗口：审批群/值日策略源/回答表范围/播报群全景只读）',
      ],
      commands: [
        '/help /lottery /<触发词>（动态抽奖指令） /status /test-ddl /keywords /autoreply /history（后五个为运维指令，不在群聊 /help 展示）',
      ...
      permissions: [
        ...
        '抽奖：@机器人 发「/触发词」即抽一次（触发词=指令名，别名均可；值日管辖群可用；审批群不开放；私聊按指令白名单口径）；LOTTERY_CHAT_IDS 可收窄；动态进 /help',
      ],
