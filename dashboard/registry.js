...
module.exports = {
  ...
  projects: [
    ...
    },
    {
      ...
      deploy: 'npm run push',
      role: '财务审批域：审批群 /approval-*、每周财务催办周报（催发票/报销单/转账）、每日待审批提醒、催发票私聊、发票图像 OCR 转录。',
      listening: ['不消费消息事件（hub 转发 /api/chat/command）', 'p2p 回复轮询（催发票延期/无法提交识别）', 'GET /api/approval/policy（定制窗口：流程/审批人白名单/催办参数全景只读）', 'POST /api/ocr/transcribe（发票 OCR 转录，飞书免费 OCR）'],
      ...
      permissions: ['仅服务审批群（BOT_CHAT_ID）', '定时任务：周报周一 18:00 / 每日待审批提醒 09:00 / 催发票私聊每日 10:30（宽限 14 天、每 2 天一催、上限 5 次、回复「延期 N 天/无法提交」；均过静默闸门）', '发票 OCR（飞书官方接口，免费 20QPS）：需飞书后台开通 im:resource + optical_char_recognition 两权限'],
      localRun: { script: 'src/index.js', cwd: '', env: {} },
      quickActions: [
        { id: 'install', label: 'npm install', cmd: 'npm install', cwd: '' },
      ],
      notes: '提醒回落 open_id 已改通用变量名（REMINDER_FALLBACK_OPEN_ID_1/2）；v44 发票 OCR：字段规则存 .ocr-fields.local.json（生产配 OCR_FIELDS_FILE 到项目外，SFTP 部署清目录），消息侧入口待定',
    },
    {
      id: 'ticket',
      name: 'ticket-bot',
