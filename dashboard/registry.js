...
module.exports = {
  ...
  projects: [
    ...
    },
    {
      ...
      role: '企业微信考勤域：负责人群每周播报考勤打卡数据（markdown_v2 周报卡 + CSV 明细附件），拉企微打卡 API 聚合，不碰考勤机硬件。',
      listening: [
        '不消费任何消息事件（飞书/企微都不收，纯定时任务）',
        '出站：企微 gettoken + checkin/getcheckindata（受自建应用可信 IP 限制）',
        '出站：群机器人 webhook/send + upload_media（key 即凭证，不走可信 IP，拉数失败也用它告警）',
        '补发看门狗：每小时 5 分对表，漏播/失败自动补（水位 lastSentWeekKey）',
      ],
      ...
      notes: 'v1 诞生（2026-09-15）：企微打卡 API 约束=拉数跨度≤30天/userid 单批≤100（自动分批）/webhook 20条每分；拉数需自建应用+「打卡-可调用接口的应用」授权+可信 IP（家宽 IP 变动报 60020，告警卡会提示；webhook 播报不受可信 IP 限制）；周报用 markdown_v2（旧客户端表格退化纯文本，CSV 兜底）；「缺卡判定」需打卡规则接口对照班次，留 v2',
    },
    {
      id: 'dashboard',
